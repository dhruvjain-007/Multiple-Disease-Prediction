...
.stButton > button:hover {
    background-color: #5f6d2e !important;
}
...
input, textarea {
    ...
    color: #2f3a17 !important;
    border: 1px solid #c5b894 !important;
    border-radius: 8px !important;
}
...
/* SUCCESS BOX */
[data-testid="stAlert"] {
    background-color: #e6f0d4 !important;
    color: #2f3a17 !important;
}
...
if selected == 'Symptom Analysis': 
    # Create disease class and load ML model
    disease_model = DiseaseModel()
    disease_model.load_xgboost('model/xgboost_model.json')

    # Title
    st.write('# Health Condition Analysis using AI')
    ...
    if st.button('Analyze Symptoms'): 
        ...
        st.write(f'## Potential Condition: {prediction} with {prob*100:.2f}% confidence')
...
if selected == 'Metabolic Disorder Check':  # pagetitle
    st.title("Metabolic Disorder Assessment")
    # columns
    ...
    # button
    if st.button("Run Metabolic Assessment"):
        diabetes_prediction=[[]]
        ...
        if diabetes_prediction[0] == 1:
            diabetes_dig = "Assessment indicates potential metabolic disorder."
        else:
            diabetes_dig = 'Assessment shows no metabolic disorder detected.'
...
if selected == 'Cardiovascular Risk Assessment':
    st.title("Cardiovascular Risk Evaluation")
    ...
    # button
    if st.button("Evaluate Cardiovascular Risk"):
        heart_prediction=[[]]
        ...
        if heart_prediction[0] == 1:
            heart_dig = 'Evaluation suggests cardiovascular risk present.'
        else:
            heart_dig = "Evaluation indicates no cardiovascular risk detected."
...
if selected == 'Movement Disorder Evaluation':
    st.title("Movement Disorder Screening")
  # parameters
#    name	MDVP:Fo(Hz)	MDVP:Fhi(Hz)	MDVP:Flo(Hz)	MDVP:Jitter(%)	MDVP:Jitter(Abs)	MDVP:RAP	MDVP:PPQ	Jitter:DDP	MDVP:Shimmer	MDVP:Shimmer(dB)	Shimmer:APQ3	Shimmer:APQ5	MDVP:APQ	Shimmer:DDA	NHR	HNR	status	RPDE	DFA	spread1	spread2	D2	PPE
   # change the variables according to the dataset used in the model
    ...
    # button
    if st.button("Run Movement Disorder Screening"):
        ...
        if parkinson_prediction[0] == 1:
            parkinson_dig = 'Screening indicates potential movement disorder.'
        else:
            parkinson_dig = "Screening shows no movement disorder detected."
...
if selected == 'Pulmonary Oncology Check':
    st.title("Pulmonary Oncology Screening")
    ...
    # Button
    if st.button("Run Pulmonary Screening"):
        ...
        if cancer_prediction[0] == 'YES':
            cancer_result = "Screening indicates potential pulmonary risk."
        else:
            cancer_result = "Screening shows no pulmonary risk detected."
...
if selected == 'Liver Function Test':  # pagetitle
...
# st.write(info.astype(int).info())
    ...
    if st.button("Analyze Liver Function"):
        ...
        if liver_prediction[0] == 1:
            liver_dig = "Analysis indicates potential liver dysfunction."
        else:
            liver_dig = "Analysis shows normal liver function."
...
if selected == 'Hepatitis Screening':
    st.title("Hepatitis Screening")
...
# st.write(info.astype(int).info())
    ...
    # button
    if st.button("Run Hepatitis Screening"):
        jaundice_prediction=[[]]
        ...
        if jaundice_prediction[0] == 1:
            jaundice_dig = "Screening indicates potential hepatitis."
        else:
            jaundice_dig = "Screening shows no hepatitis detected."
...
if selected == 'Kidney Disease Monitor':
    st.title("Kidney Disease Monitoring")
    ...
    # Button
    if st.button("Run Kidney Monitoring"):
        # Create a DataFrame with user inputs
        ...
        if kidney_prediction[0] == 1:
            kidney_prediction_dig = "Monitoring indicates potential kidney dysfunction."
        else:
            kidney_prediction_dig = "Monitoring shows normal kidney function."
